...
python scripts/prep_train_val_metadata.py | tee -a $LOGFILEPATH
train_val_meta_script_status=$?
echo

echo converting xml annotations to yolov5 format
python scripts/convert_xml_to_yolo.py | tee -a $LOGFILEPATH
...
python train.py --batch 16 --epochs 1 --data data/road.yaml --weights yolov5s.pt | tee -a $LOGFILEPATH
train_script_status=$?
echo

echo ------------- Getting Latest Training Run Folder ------------
python scripts/get_latest_run_info.py | tee -a $LOGFILEPATH
...
python test.py --weights $latest_weights_path --data data/road.yaml --task test | tee -a $LOGFILEPATH
test_perf_script_status=$?

# Get info of latest test folders
python scripts/get_latest_run_info.py --results_dir runs/test/ --tag test | tee -a $LOGFILEPATH
