...
# Function to change image file references to yolov5
def change_path_reference(filename_path):
    ...
    # Replace datasets with ../datasets
    rand2 = [item.replace("datasets/","../datasets/") for item in rand1]
    ...
    print("Modifying image path references in {} from relative to yolov5 folder as root".format(filename_path))
...
def main(input_train_file, input_valid_file, input_test_file, class_file):
    ...
    # Converting the text files to refer from yolov5 folder
    change_path_reference(input_train_file)
    change_path_reference(input_valid_file)
    change_path_reference(input_test_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--class_file', type=str,
                        ...
                        default="config/damage_classes.txt")
